...
int main() {
    ...
    do {
        ...
        printf("Digite o n̼mero associado a sua aposta: ");
    ...
    for (int i = 0; i < total_apostas; i++) {
        if (verificar_aposta(animal_aposta[i], numero_sorteado)) {
            printf("Parab̩ns! Sua aposta no animal %s com o n̼mero %d foi vencedora!\n", animal_aposta[i], numero_sorteado);
            apostas_vencedoras++; 
        } else {
            printf("Que pena! Sua aposta no animal %s com o n̼mero %d ṇo foi vencedora.\n", animal_aposta[i], numero_sorteado);
        }
    }
